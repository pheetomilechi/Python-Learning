# 1. Standard division always returns a float (decimal) while floor division returns an integer (whole number) result. 11 / 2 = 5.5, 11 // 2 = 5
# 2. Modulo operator returns the remainder of a division operation. 12 % 5 = 2 
# 3. Because age is a variable and 20 is an integer, the comparison operator < is used to check if the value of age is less than 20. If age is less than 20, the condition evaluates to True; otherwise, it evaluates to False.
# 4. x = 5, y = 10, x > 3; returns True, y < 5; returns False. This is because the comparison operator > checks if x is greater than 3, which is true, while the comparison operator < checks if y is less than 5, which is false.
